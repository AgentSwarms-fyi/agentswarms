...
`docker compose up -d` starts one, and
...
A **deployment** holds the version it serves in memory and answers over HTTP
instead. Find it on the model page under **Automation → Warm endpoint**. While
a candidate is being shadowed or run as a canary it holds that one too, which
is what the copies below are counting.
...
- Training and inference need the notebook runtime services:
  `docker compose up -d`. The runtime image bakes the ML
...
| Real-time inference        | Warm endpoints hold the served version in memory: 45 ms of scoring instead of a ~25 s container start; several copies per endpoint, scaled on measured load, plus a candidate's copy while one is being tried                                                                                                                                                             | Serving endpoints with autoscaling                                   |
...
| --------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| "Cannot reach the Docker socket-proxy"              | Read the rest of the message: "start the runtime services" means the proxy is down (`docker compose up -d`); "did not answer" means the Docker daemon is busy or the proxy wedged — retry, then restart `notebook-docker-proxy`. |
