...
- Ships the `agentswarms` Python helper (same surface as the Pyodide one: `chat`, `kb_search`, `list_knowledge_bases`, `format_context`) pre-imported, pointed at the **in-cluster app URL** and authenticating with the injected session token — so notebooks feel identical across Lite and Server runtimes.
