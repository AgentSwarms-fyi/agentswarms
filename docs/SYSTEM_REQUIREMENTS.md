...
| **Notebook / MCP runtime**   | Sandboxed Docker containers for the Developer workspace (Python Lab) and MCP Builder.                                                                                    | Each interactive sandbox is capped at **2 GB RAM** (batch: 4 GB) and ~1 CPU by default. Size the host for _concurrent_ sandboxes, not total users.                                                                                                                                                             |
| **docgen-service**           | Python sidecar rendering PPTX/DOCX/XLSX.                                                                                                                                 | Bursty; 1 vCPU / 1–2 GB is fine for teams.                                                                                                                                                                                                                                                                     |
| **JS sandbox**               | One small container running Function nodes and custom components from deployed and scheduled swarm runs, off the app's network.                                          | ~128–256 MB. It answers requests and initiates nothing; a NetworkPolicy denies it egress entirely.                                                                                                                                                                                                             |
| **Lakehouse catalog**        | A Postgres of its own holding the lakehouse's table definitions — not its data, which is Parquet in object storage.                                                      | Small but **irreplaceable**: losing it makes every Parquet file unreadable. 1 vCPU / 1–2 GB and a 10 GB volume is plenty; see [§3a](#3a-sizing-etl-and-the-lakehouse).                                                                                                                                         |
| **Object store (MinIO)**     | Where the lakehouse's Parquet files live. One container, one volume; replace it with S3/R2/GCS by pointing `LAKEHOUSE_S3_*` there. Budget disk for your tables, not RAM. |
| **Spark cluster**            | The ETL Spark engine. Either an endpoint you run (`SPARK_PROVIDER=static`) or one driver plus executors created per run on Kubernetes (`k8s`).                           | Sized per run, not per deployment — executors × cores × memory from Admin → Developer runtime. The image is ~1 GB and downloads connector jars on first use. See [§3a](#3a-sizing-etl-and-the-lakehouse).                                                                                                      |
| **Vector store**             | Qdrant, when `VECTOR_STORE=qdrant`. Holds knowledge-base embeddings only; the chunk text stays in Postgres.                                                              | **RAM is the whole point**: an HNSW index is roughly 4 bytes × 1536 dimensions × chunks, plus the graph — about 6 GB per million chunks. Running it here is how that stops competing with the database. Rebuildable, so it needs no backup.                                                                    |
| **Online feature store**     | Valkey, when `FEATURE_STORE_URL` names it. Holds a feature view's latest row per key so a prediction does not read the lakehouse for it.                                 | **Bounded on purpose**: `FEATURE_STORE_MAXMEMORY` (256 MB default) with `allkeys-lru`, so it cannot grow past what you gave it — roughly a few hundred bytes per key. Holds no source of truth and keeps nothing on disk; losing it costs latency, never data, because every miss falls back to the lakehouse. |
| **ML training / prediction** | Batch sandboxes of the same notebook runtime (scikit-learn, LightGBM, statsmodels on CPU), one per training job or batch prediction; artifacts in the lake bucket.       | Each training sandbox may use up to `ML_TRAIN_MEM_LIMIT_MB` (**8 GB** default) and `batch_cpu_limit` CPU for up to its time budget. Size for _concurrent_ trainings (`ML_MAX_CONCURRENT_TRAININGS_PER_USER`); **no GPU needed** — `ML_TRAIN_GPUS` is for custom CUDA images.                                   |
...
| **+ Notebook/MCP runtime** (installed by default)         | 4 vCPU  | 8 GB  | 40 GB      | Adds Docker sandboxes; each active notebook takes up to 2 GB.                                                                                         |
| **+ ML training** (same runtime)                          | 4 vCPU  | 16 GB | 40 GB      | One training sandbox at a time at the 8 GB default; the sample table trains in about a minute. Lower `ML_TRAIN_MEM_LIMIT_MB` to 4096 on an 8 GB host. |
| **+ Vector store** (installed by default)                 | +1 vCPU | +2 GB | +10 GB     | Only when `VECTOR_STORE=qdrant`. Budget ~6 GB of RAM per million chunks; below that the default pgvector is the cheaper answer.                       |
