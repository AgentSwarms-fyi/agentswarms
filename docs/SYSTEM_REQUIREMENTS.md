...
AgentSwarms is deliberately light to host. Understanding *why* makes every
...
| **LLM calls** | External by default (BYOK: OpenRouter, OpenAI, Anthropic, Bedrock, …). | **No GPU needed.** Your cost here is *tokens*, not hardware — see [§4](#4-token-budgets). GPUs only enter the picture if you self-host models ([§5](#5-gpu-sizing-self-hosted-models-only)). |
| **Notebook / MCP runtime** *(optional)* | Sandboxed Docker containers for the Developer workspace (Python Lab) and MCP Builder. | Each interactive sandbox is capped at **2 GB RAM** (batch: 4 GB) and ~1 CPU by default. Size the host for *concurrent* sandboxes, not total users. |
| **docgen-service** *(optional)* | Python sidecar rendering PPTX/DOCX/XLSX. | Bursty; 1 vCPU / 1–2 GB is fine for teams. |
...
*(Or $0 on OCI Always Free + Supabase Free.)*
...
*Prices verified against public list pricing in early 2026 and rounded.
If a number here disagrees with a cloud calculator, the calculator wins —
and a PR fixing this page is welcome.*
