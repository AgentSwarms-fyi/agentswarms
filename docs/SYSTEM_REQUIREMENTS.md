...
| **JS sandbox** _(optional)_               | One small container running Function nodes and custom components from deployed and scheduled swarm runs, off the app's network.                                    | ~128–256 MB. It answers requests and initiates nothing; a NetworkPolicy denies it egress entirely.                                                                                                                                                                                                             |
| **Lakehouse catalog** _(optional)_        | A Postgres of its own holding the lakehouse's table definitions — not its data, which is Parquet in object storage.                                                | Small but **irreplaceable**: losing it makes every Parquet file unreadable. 1 vCPU / 1–2 GB and a 10 GB volume is plenty; see [§3a](#3a-sizing-etl-and-the-lakehouse).                                                                                                                                         |
| **Spark cluster** _(optional)_            | The ETL Spark engine. Either an endpoint you run (`SPARK_PROVIDER=static`) or one driver plus executors created per run on Kubernetes (`k8s`).                     | Sized per run, not per deployment — executors × cores × memory from Admin → Developer runtime. The image is ~1 GB and downloads connector jars on first use. See [§3a](#3a-sizing-etl-and-the-lakehouse).                                                                                                      |
| **Vector store** _(optional)_             | Qdrant, when `VECTOR_STORE=qdrant`. Holds knowledge-base embeddings only; the chunk text stays in Postgres.                                                        | **RAM is the whole point**: an HNSW index is roughly 4 bytes × 1536 dimensions × chunks, plus the graph — about 6 GB per million chunks. Running it here is how that stops competing with the database. Rebuildable, so it needs no backup.                                                                    |
| **Online feature store** _(optional)_     | Valkey, when `FEATURE_STORE_URL` names it. Holds a feature view's latest row per key so a prediction does not read the lakehouse for it.                           | **Bounded on purpose**: `FEATURE_STORE_MAXMEMORY` (256 MB default) with `allkeys-lru`, so it cannot grow past what you gave it — roughly a few hundred bytes per key. Holds no source of truth and keeps nothing on disk; losing it costs latency, never data, because every miss falls back to the lakehouse. |
...
| **+ Vector store** (`--profile vectors`)                  | +1 vCPU | +2 GB | +10 GB     | Only when `VECTOR_STORE=qdrant`. Budget ~6 GB of RAM per million chunks; below that the default pgvector is the cheaper answer.                       |
