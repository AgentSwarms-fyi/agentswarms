...
| 🔁 **ETL Pipelines**             | Move data between systems under **Data & BI → ETL Pipelines**: a Glue-style **visual canvas** (sources → joins/aggregates/transforms → targets, with the compiled Python one toggle away), a full code mode, and **AI generate/refine with your own provider and model**. Sources: S3-compatible storage (CSV, TSV, JSON, JSONL, Parquet, Excel), PostgreSQL/MySQL/SQL Server-family databases, HTTP APIs, custom Python. Targets: object storage (Parquet/CSV/JSONL) and the same database families, with replace/append/merge. Runs execute on the sandboxed runtime — credentials reach process memory only, never code or container env — on a schedule or webhook trigger, and every successful load **re-crawls the destination** so new tables appear in the Data Catalog for BI, the AI Analyst and agents. See **[ETL pipelines](./docs/ETL_PIPELINES.md)**.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| **[ETL pipelines](./docs/ETL_PIPELINES.md)**                             | Visual-canvas and code pipelines with AI generation: supported sources and targets, the execution model on the sandboxed runtime, credentials, scheduling, the webhook trigger, and the catalog re-crawl that makes loaded data queryable everywhere.                                                                           |
