...
| 🗂️ **Data Catalog**          | Connect warehouses or S3-compatible buckets (AWS S3, Cloudflare R2, MinIO, Spaces, B2) through a wizard; the crawler lists every table and object, groups partitioned folders into datasets, infers CSV/JSON schemas by sampling, profiles columns (null %, distinct counts, ranges), estimates row counts, and flags likely-PII columns. Schedule daily/weekly incremental crawls with schema-drift notifications, generate asset + column documentation with AI, certify or deprecate assets with owners and tags, and jump straight into the SQL workbench. |
