...
| 🗂️ **Data Catalog**          | Connect warehouses or S3-compatible buckets (AWS S3, Cloudflare R2, MinIO, Spaces, B2) through a wizard; the crawler lists every table and object, groups partitioned folders into datasets, infers CSV/JSON schemas by sampling, estimates row counts, and flags likely-PII columns. Search, tag and describe assets, then jump straight into the SQL workbench. |
