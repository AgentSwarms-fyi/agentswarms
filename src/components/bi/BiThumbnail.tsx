  ...
  switch (type) {
    ...
    case "combo":
      return (
        <svg viewBox="0 0 40 24" preserveAspectRatio="none" className="h-full w-full">
          {type === "area" && (
            <path d="M2,20 L10,11 L18,15 L26,6 L34,10 L38,10 L38,22 L2,22 Z" fill={c} opacity="0.25" />
          )}
          <polyline
        ...
        <div className="flex w-full flex-col gap-[3px] px-2">
          {[0.9, 0.6, 0.75, 0.5].map((w, i) => (
            <div
              ...
              style={{ width: `${w * 100}%`, background: i === 0 ? c : "currentColor", opacity: i === 0 ? 0.8 : 0.22 }}
            />
          ))}
        </div>
      );
    ...
    case "text":
      return (
        <div className="flex w-full flex-col gap-[3px] px-2">
          {[1, 0.85, 0.6].map((w, i) => (
            <div key={i} className="h-[3px] rounded-full bg-current opacity-30" style={{ width: `${w * 100}%` }} />
          ))}
        </div>
      );
    case "map":
    case "bubblemap":
      return (
        <svg viewBox="0 0 32 24" className="h-[85%]">
          <path
            d="M6,6 Q10,2 16,5 T28,6 Q30,12 24,16 T12,18 Q4,14 6,6 Z"
            fill={c}
            opacity="0.3"
          />
      ...
      return (
        <div className="flex w-full flex-col justify-center gap-[3px] px-2">
          {[0.9, 0.65, 0.8, 0.45].map((w, i) => (
            <div key={i} className="h-[3px] rounded-full" style={{ width: `${w * 100}%`, background: c, opacity: 0.85 - i * 0.12 }} />
          ))}
        </div>
      );
    default:
      ...
      return (
        <div className="flex h-[80%] w-full items-end justify-center gap-[3px] px-1">
          {[0.5, 0.85, 0.35, 0.7, 0.6].map((h, i) => (
            <div key={i} className="w-[14%] rounded-t-[2px]" style={{ height: `${h * 100}%`, background: c, opacity: 0.85 - i * 0.06 }} />
          ))}
        </div>
      );
  }
