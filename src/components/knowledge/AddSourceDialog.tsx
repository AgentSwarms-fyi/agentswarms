...
export function AddSourceDialog({ open, onOpenChange, knowledgeBaseId, userId, onAdded }: Props) {
  ...
  const onDropFiles = useCallback((accepted: File[], rejected: { file?: File; errors?: { message?: string }[] }[]) => {
    rejected?.forEach((r) => {
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    onDrop: onDropFiles,
    validator: (file: File) => {
      const allowed = [".txt", ".md", ".markdown", ".csv", ".tsv", ".log", ".html", ".htm", ".xml", ".yaml", ".yml", ".json", ".rtf", ".pdf", ".docx"];
  ...
  function reset() {
    setUrl(""); setUrlLabel("");
    setRepo(""); setBranch(""); setGlobs("*.md, *.mdx, *.txt");
    ...
    try {
      if (tab === "url") {
        ...
        const res = await fetch("/api/kb/ingest-url", {
          ...
          body: JSON.stringify({ knowledge_base_id: knowledgeBaseId, url: url.trim(), label: urlLabel.trim() || undefined }),
        });
        ...
        if (!repo.match(/^[\w.-]+\/[\w.-]+$/)) { toast.error("repo must be owner/name"); return; }
        const globList = globs.split(",").map((s) => s.trim()).filter(Boolean);
        const { data: sessionData } = await supabase.auth.getSession();
        const token = sessionData.session?.access_token;
        ...
        const json = await res.json().catch(() => ({}));
        if (!res.ok) { toast.error(json.error || `Ingest failed (${res.status})`); return; }
        toast.success(`Imported ${json.documents} file${json.documents === 1 ? "" : "s"} from ${repo}@${json.branch}`);
        reset(); onAdded(); onOpenChange(false);
      } else if (tab === "manual") {
        if (!manualName.trim() || !manualBody.trim()) { toast.error("Name and content are required"); return; }
        ...
        const { data: src, error: srcErr } = await supabase
          ...
          .select("id")
          .single();
        if (srcErr || !src) { toast.error(srcErr?.message || "Could not record source"); return; }
        const { data: insertedDoc, error: docErr } = await supabase
          ...
          .select("id")
          .single();
        if (docErr || !insertedDoc) { toast.error(docErr?.message || "Insert failed"); return; }
        try {
        ...
        // file
        if (files.length === 0) { toast.error("Drop at least one file"); return; }
        ...
        for (const f of files) {
          ...
          const { data: src, error: srcErr } = await supabase
            ...
            .select("id")
            .single();
          if (srcErr || !src) { toast.error(srcErr?.message || `Could not record source for ${f.name}`); continue; }
      ...
      <DialogContent className="max-w-xl max-h-[90vh] overflow-y-auto">
        ...
        <Tabs value={tab} onValueChange={(v) => setTab(v as typeof tab)}>
          <TabsList className="w-full">
            <TabsTrigger value="file" className="flex-1"><UploadCloud className="h-3 w-3 mr-1" /> File</TabsTrigger>
            <TabsTrigger value="url" className="flex-1"><Globe className="h-3 w-3 mr-1" /> URL</TabsTrigger>
            <TabsTrigger value="github" className="flex-1"><GitBranch className="h-3 w-3 mr-1" /> GitHub</TabsTrigger>
            <TabsTrigger value="manual" className="flex-1"><FileText className="h-3 w-3 mr-1" /> Manual</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="file" className="space-y-3 mt-4">
            ...
            >
              ...
              <p className="text-xs text-muted-foreground mt-1">PDF, DOCX, TXT, MD, CSV, JSON, HTML — up to 50MB each</p>
            </div>
            {files.length > 0 && (
              <div className="space-y-1 max-h-40 overflow-y-auto">
                {files.map((f, i) => (
                  <div key={i} className="flex items-center justify-between rounded-md border border-border/60 bg-muted/30 px-2 py-1.5 text-xs">
          ...
          <TabsContent value="url" className="space-y-3 mt-4">
            <div className="space-y-1">
              ...
              <Input value={url} onChange={(e) => setUrl(e.target.value)} placeholder="https://example.com/docs/intro" />
            </div>
            <div className="space-y-1">
              <Label className="text-xs">Label (optional)</Label>
              <Input value={urlLabel} onChange={(e) => setUrlLabel(e.target.value)} placeholder="Defaults to page title" />
            </div>
          ...
          <TabsContent value="github" className="space-y-3 mt-4">
            <div className="space-y-1">
              ...
              <Input value={repo} onChange={(e) => setRepo(e.target.value)} placeholder="vercel/next.js" className="font-mono text-xs" />
            </div>
            <div className="grid grid-cols-2 gap-2">
              ...
              <div className="space-y-1">
                ...
                <Input value={globs} onChange={(e) => setGlobs(e.target.value)} placeholder="*.md, *.mdx, *.txt" className="font-mono text-xs" />
              </div>
            </div>
            <p className="text-[11px] text-muted-foreground">
              Walks the repo's git tree and imports up to 50 matching files (≤1 MB each). Public repos work without auth; rate-limited to 60 reqs/h unless GITHUB_TOKEN is configured.
            </p>
          </TabsContent>
          ...
          <TabsContent value="manual" className="space-y-3 mt-4">
            <div className="space-y-1">
              ...
              <Input value={manualName} onChange={(e) => setManualName(e.target.value)} placeholder="Product FAQ" />
            </div>
            <div className="space-y-1">
              <Label className="text-xs">Content</Label>
              <Textarea rows={8} value={manualBody} onChange={(e) => setManualBody(e.target.value)} placeholder="Paste your text here…" />
            </div>
          </TabsContent>
        </Tabs>

        <Button onClick={handleSubmit} disabled={busy} className="w-full mt-4">
          {busy ? <><Loader2 className="h-4 w-4 mr-2 animate-spin" /> Working…</> : "Add source"}
        </Button>
      </DialogContent>
    </Dialog>
  );
