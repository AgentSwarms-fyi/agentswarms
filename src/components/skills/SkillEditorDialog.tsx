...
type Props = {
  ...
  initial?: { id: string; name: string; description: string | null; body: string; tags: string[] } | null;
};
...
export function SkillEditorDialog({ open, onOpenChange, onSaved, initial }: Props) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const data = (await resp.json()) as { name?: string; description?: string; body?: string; tags?: string[] };
        ...
        <Tabs value={tab} onValueChange={setTab}>
          ...
          <TabsContent value="ai" className="space-y-3 pt-3">
            <p className="text-xs text-muted-foreground">
              ...
              <em> When to use</em>, <em>Instructions</em>, <em>Examples</em>, and <em>Constraints</em>.
            </p>
            <Textarea
              rows={6}
              value={brief}
          ...
          <TabsContent value="manual" className="space-y-3 pt-3">
            ...
            <div className="space-y-1.5">
              ...
              <p className="text-[11px] text-muted-foreground">
                The full markdown body is injected into the agent's system prompt at run time.
                Keep it focused — one capability per skill.
              </p>
            </div>
          </TabsContent>
        </Tabs>
