...
const TRACE_LIST_COLUMNS = "id, agent_name, llm_provider, llm_model, latency_ms, tokens_in, tokens_out, cost_usd, status, prompt, error_message, created_at";

export const getExecutionTraces = createServerFn({ method: "POST" })
  .inputValidator((input: unknown) => TraceLogInput.parse(input))
  .handler(async ({ data }): Promise<{ ok: true; traces: ExecutionTraceRow[] } | { ok: false; error: string }> => {
...
export const getExecutionTraceDetail = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<{ ok: true; trace: ExecutionTraceRow | null } | { ok: false; error: string }> => {
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
