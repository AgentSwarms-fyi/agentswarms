...
export const AGENTIC_PATTERNS: AgenticPattern[] = [
  // 1. ReAct ───────────────────────────────────────────────────────────────
  {
    ...
    nodes: [
      {
        id: "prompt",
        label: "User Prompt",
        sublabel: "Question / task",
        kind: "input",
        icon: User,
        position: { x: X.col1, y: Y.row2 },
      },
      {
        id: "thought",
        label: "Thought",
      ...
      },
      {
        ...
        kind: "observation",
        icon: Eye,
        position: { x: X.col4, y: Y.row2 },
      },
      {
        id: "final",
        label: "Final Answer",
        sublabel: "Returned to user",
        kind: "output",
        icon: CheckCircle2,
        position: { x: X.col5, y: Y.row2 },
      },
    ],
    edges: [
    ...
    tour: [
      ...
      },
      {
        ...
        realWorld:
          "Claude's <thinking> blocks and OpenAI's reasoning summaries are direct descendants of this idea.",
      },
      {
        nodeIds: ["action"],
        edgeIds: ["e3"],
        ...
        realWorld:
          "Function calling in OpenAI, tool_use in Anthropic, and Gemini's function-calling all expose this exact step.",
      },
      {
        nodeIds: ["observation"],
        edgeIds: ["e4"],
        ...
        realWorld:
          "Every agentic browser, code assistant, and research agent loops here, often 3–8 times for a single user question.",
      },
      {
        nodeIds: ["final"],
        edgeIds: ["e5"],
        ...
        realWorld:
          "LangChain's AgentExecutor has `max_iterations`. OpenAI Assistants enforce step limits. Always set one.",
      },
    ],
  },
  ...
  // 2. Reflection ──────────────────────────────────────────────────────────
  {
    ...
    nodes: [
      ...
      },
      {
        id: "revise",
        label: "Revise",
        sublabel: "Rewrite with feedback",
        kind: "reviser",
        icon: RefreshCw,
        position: { x: X.col4, y: Y.row2 },
      },
    ...
    tour: [
      {
        ...
        realWorld:
          "This is exactly how Cursor's 'fast apply' and Notion AI generate drafts before refinement.",
      },
      {
        nodeIds: ["critique"],
        edgeIds: ["e2"],
        ...
        realWorld:
          "Anthropic's Constitutional AI critiques against an explicit list of principles. Same idea here.",
      },
      {
        nodeIds: ["revise"],
        edgeIds: ["e3"],
  ...
  // 3. Planning ────────────────────────────────────────────────────────────
  {
    ...
    nodes: [
      ...
      },
      {
        id: "planner",
        label: "Planner Agent",
        sublabel: "Breaks goal into steps",
        kind: "planner",
        icon: ClipboardList,
        position: { x: X.col2, y: Y.row2 },
      },
      ...
      {
        id: "exec1",
        label: "Executor · Task 1",
        sublabel: "Runs first subtask",
        kind: "worker",
        icon: Bot,
        position: { x: X.col4, y: Y.row1 },
      },
      {
        id: "exec2",
        label: "Executor · Task 2",
        sublabel: "Runs next subtask",
        kind: "worker",
        icon: Bot,
        position: { x: X.col4, y: Y.row3 },
      },
      {
        id: "synth",
        label: "Synthesizer",
        sublabel: "Combines results",
        kind: "synthesizer",
        icon: Layers,
        position: { x: X.col5, y: Y.row2 },
      },
    ...
    tour: [
      {
        ...
        realWorld:
          "Devin's planner is a separate model call from its execution agents — by design.",
      },
      {
        nodeIds: ["planner", "tasks"],
        edgeIds: ["e2"],
      ...
      },
      {
        ...
        realWorld:
          "Manus (the autonomous browsing agent) parallelizes independent research subtasks aggressively.",
      },
      {
        nodeIds: ["exec1", "exec2", "synth"],
        edgeIds: ["e5", "e6"],
  ...
  // 4. Tool Use ────────────────────────────────────────────────────────────
  {
    ...
    name: "Tool Use",
    tagline:
      "The LLM picks from a registry of tools (APIs, DBs, code) and the runtime executes them.",
    accent: "sky",
    summary:
    ...
    nodes: [
      ...
      },
      {
        ...
        kind: "output",
        icon: CheckCircle2,
        position: { x: X.col5 + 220, y: Y.row2 },
      },
    ],
    edges: [
    ...
    tour: [
      {
        ...
        realWorld:
          "MCP (Model Context Protocol) is essentially a standard for sharing tool catalogs between agents and external systems.",
      },
      {
        nodeIds: ["llm", "router"],
        edgeIds: ["e2"],
      ...
      },
      {
        ...
        realWorld:
          "This is exactly where guardrails like Cloudflare AI Gateway and Portkey sit in real deployments.",
      },
      {
        nodeIds: ["llm2"],
        edgeIds: ["e6", "e7", "e8"],
  ...
  // 5. Multi-Agent Collaboration ───────────────────────────────────────────
  {
    ...
    nodes: [
      ...
      },
      {
        id: "researcher",
        label: "Researcher",
        sublabel: "Gathers facts",
        kind: "agent",
        icon: Search,
        position: { x: X.col3, y: Y.row1 },
      },
      {
        id: "analyst",
        label: "Analyst",
        sublabel: "Crunches numbers",
        kind: "agent",
        icon: Calculator,
        position: { x: X.col3, y: Y.row3 },
      },
      {
        id: "writer",
        label: "Writer",
        sublabel: "Drafts the output",
        kind: "agent",
        icon: FileText,
        position: { x: X.col4, y: Y.row1 },
      },
    ...
    tour: [
      ...
      {
        nodeIds: ["orch", "researcher", "analyst"],
        edgeIds: ["e2", "e3"],
        ...
        realWorld:
          "Bloomberg's GPT (BloombergGPT) deployments use specialist agents per asset class.",
      },
      {
        nodeIds: ["researcher", "analyst", "writer"],
        edgeIds: ["e4", "e5"],
  ...
  // 6. Sequential Workflow ─────────────────────────────────────────────────
  {
    ...
    tour: [
      ...
      },
      {
        ...
        realWorld:
          "Stripe Radar, Notion's AI summary, and Linear's auto-triage are sequential workflows under the hood.",
      },
      {
        nodeIds: ["s2", "s3"],
        edgeIds: ["e3"],
  ...
  // 8. Agentic RAG (Self-Correcting Retrieval) ─────────────────────────────
  {
    ...
    tagline:
      "RAG with a feedback loop: grade retrieved docs, rewrite the query, retry until grounded.",
    accent: "indigo",
    summary:
    ...
    nodes: [
      ...
      },
      {
        ...
        sublabel: "LLM scores each chunk",
        kind: "critique",
        icon: GitCompare,
        position: { x: X.col3, y: Y.row2 },
      },
      {
        ...
        sublabel: "Reformulate & retry",
        kind: "llm",
        icon: RefreshCw,
        position: { x: X.col2, y: Y.row4 },
      },
      {
        id: "websearch",
        label: "Web Fallback",
        sublabel: "If KB has no good docs",
        kind: "tool",
        icon: Search,
        position: { x: X.col3, y: Y.row4 },
      },
      {
        id: "generate",
        label: "Generate Answer",
        sublabel: "Grounded in good chunks",
        kind: "synthesizer",
        icon: Brain,
        position: { x: X.col4, y: Y.row2 },
      },
    ...
    tour: [
      {
        ...
        realWorld:
          "Perplexity, Notion AI, and most enterprise 'chat with your docs' products all start at this exact step.",
      },
      {
        nodeIds: ["retrieve", "grader"],
        edgeIds: ["e2"],
      ...
      {
        nodeIds: ["grader", "rewrite", "retrieve"],
        edgeIds: ["e4", "e5"],
      ...
      {
        nodeIds: ["grader", "websearch", "generate"],
        edgeIds: ["e6", "e7"],
        ...
        realWorld:
          "Self-RAG (Asai et al., 2023) introduced this 'reflect on retrieval, then optionally search externally' loop.",
      },
      {
        nodeIds: ["generate", "final"],
        edgeIds: ["e8"],
  ...
  // 9. Supervisor (Hierarchical Multi-Agent) ───────────────────────────────
  {
    ...
    nodes: [
      ...
      },
      {
        ...
        sublabel: "Web + KB lookups",
        kind: "agent",
        icon: Search,
        position: { x: X.col5, y: Y.row1 },
      },
      {
        id: "code",
        label: "Code Agent",
        sublabel: "Writes & runs code",
        kind: "agent",
        icon: Wrench,
        position: { x: X.col5, y: Y.row2 },
      },
      {
        id: "math",
        label: "Math Agent",
        sublabel: "Numerical work",
        kind: "agent",
        icon: Calculator,
        position: { x: X.col5, y: Y.row3 },
      },
      {
        id: "final",
        label: "Final Answer",
        sublabel: "Supervisor responds",
        kind: "output",
        icon: CheckCircle2,
        position: { x: X.col1, y: Y.row4 },
      },
    ],
    edges: [
    ...
    tour: [
      ...
      {
        nodeIds: ["supervisor", "research", "code", "math"],
        edgeIds: ["e2", "e3", "e4"],
        ...
        realWorld:
          "LangGraph's supervisor uses structured output (Pydantic schema) so the routing decision is type-safe.",
      },
      {
        nodeIds: ["research", "code", "math", "supervisor"],
        edgeIds: ["e5", "e6", "e7"],
      ...
      },
      {
        ...
        realWorld:
          "OpenAI's Agents SDK calls this 'final response from the orchestrator' and recommends a max-step cap to prevent runaway routing loops.",
      },
    ],
  },
  ...
  // 10. Swarm / Handoff (Decentralized Multi-Agent) ────────────────────────
  {
    ...
    tagline:
      "Agents transfer the conversation directly to a peer when their job is done — no central manager.",
    accent: "rose",
    summary:
    ...
    nodes: [
      {
        id: "user",
        label: "User",
        sublabel: "Talks to one agent at a time",
        kind: "input",
        icon: User,
        position: { x: X.col1, y: Y.row2 },
      },
    ...
    tour: [
      ...
      },
      {
        ...
        realWorld:
          "OpenAI's Agents SDK literally models this as `agent.handoffs = [sales_agent, support_agent]`.",
      },
      {
        nodeIds: ["support", "billing", "sales"],
        edgeIds: ["e4", "e5"],
      ...
      {
        nodeIds: ["billing", "final", "sales", "support"],
        edgeIds: ["e6", "e7", "e8"],
  ...
  // 11. Tree of Thoughts (Deliberate Search) ────────────────────────────────
  {
    ...
    tagline:
      "Explore multiple reasoning branches in parallel, score them, and prune the weak ones.",
    accent: "teal",
    summary:
    ...
    nodes: [
      ...
      {
        id: "expand",
        label: "Branch Expander",
        sublabel: "Generate N candidates",
        kind: "llm",
        icon: TreePine,
        position: { x: X.col2, y: Y.row2 },
      },
      ...
      },
      {
        id: "evaluator",
        label: "Evaluator",
        sublabel: "Score each branch 0-1",
        kind: "critique",
        icon: GitCompare,
        position: { x: X.col4, y: Y.row2 },
      },
    ...
    tour: [
      ...
      },
      {
        ...
        realWorld:
          "Yao et al. used branching factor 5 for Game-of-24 and beam width 1 — meaning explore 5, keep the best 1.",
      },
      {
        nodeIds: ["b1", "b2", "b3", "evaluator"],
        edgeIds: ["e5", "e6", "e7"],
  ...
  // 12. Memory-Augmented Agent ─────────────────────────────────────────────
  {
    ...
    tagline:
      "Short-term, episodic, semantic, and procedural memory — agents that learn across sessions.",
    accent: "amber",
    summary:
    ...
    nodes: [
      ...
      },
      {
        id: "short",
        label: "Short-Term",
        sublabel: "Current chat (in context)",
        kind: "step",
        icon: Repeat,
        position: { x: X.col2, y: Y.row1 },
      },
      {
        id: "episodic",
        label: "Episodic Memory",
        sublabel: "Past conversations",
        kind: "step",
        icon: Archive,
        position: { x: X.col2, y: Y.row3 },
      },
      {
        id: "semantic",
        label: "Semantic Memory",
        sublabel: "Facts about user / domain",
        kind: "step",
        icon: BookMarked,
        position: { x: X.col2, y: Y.row4 },
      },
      {
        ...
        sublabel: "Learned prompts / behaviors",
        kind: "step",
        icon: Workflow,
        position: { x: X.col4, y: Y.row1 },
      },
    ...
    tour: [
      ...
      },
      {
        ...
        realWorld:
          "ChatGPT's user-visible Memory feature is exactly this: surfaced semantic facts.",
      },
      {
        nodeIds: ["procedural", "agent"],
        edgeIds: ["e5"],
      ...
      {
        nodeIds: ["agent", "writer", "episodic", "semantic"],
        edgeIds: ["e7", "e8", "e9"],
  ...
  // 13. LLM Router (Cost / Capability Routing) ─────────────────────────────
  {
    ...
    nodes: [
      ...
      },
      {
        ...
        sublabel: "Code + structured edits",
        kind: "llm",
        icon: Wrench,
        position: { x: X.col4, y: Y.row2 },
      },
      {
        ...
        sublabel: "Hard reasoning, agentic",
        kind: "llm",
        icon: Brain,
        position: { x: X.col4, y: Y.row3 },
      },
      {
        id: "fallback",
        label: "Escalation",
        sublabel: "Retry on harder model",
        kind: "checkpoint",
        icon: ArrowRight,
        position: { x: X.col3, y: Y.row4 },
      },
    ...
    tour: [
      {
        ...
        realWorld:
          "vLLM Semantic Router uses sentence embeddings + a learned classifier. Portkey and Cloudflare AI Gateway use rule-based + LLM classifiers.",
      },
      {
        nodeIds: ["classifier", "nano"],
        edgeIds: ["e2"],
      ...
      {
        nodeIds: ["nano", "fallback", "frontier"],
        edgeIds: ["e8", "e9"],
        ...
        realWorld:
          "OpenRouter's auto-router uses confidence + provider availability to decide when to retry on a different model.",
      },
      {
        nodeIds: ["nano", "code", "frontier", "answer"],
        edgeIds: ["e5", "e6", "e7"],
  ...
  // 14. Responsible AI / Guardrails (Branching: Unsafe vs Safe) ─────────────
  {
    ...
    tagline:
      "Branching swarm: an unguarded autonomous agent vs. a Guardrail/Audit chain that intercepts, redacts, and explains.",
    accent: "emerald",
    summary:
    ...
    nodes: [
      // Shared intake (slate)
      {
        ...
        sublabel: "Jane Doe · $85k · 90210",
        kind: "input",
        icon: FileText,
        position: { x: X.col1, y: 220 },
      },
      {
        id: "rai_intake",
        label: "Data Intake Agent",
        sublabel: "Parses raw applicant data",
        kind: "agent",
        icon: Download,
        position: { x: X.col2, y: 220 },
      },

      // BRANCH A — Unsafe path (top, amber/red)
      {
        id: "rai_unrestricted",
        label: "Unrestricted Evaluator",
        sublabel: "Sees ZIP, name, demographics",
        kind: "agent",
        icon: Bot,
        position: { x: X.col3, y: 60 },
        accent: "amber",
      },
      {
        id: "rai_biased",
        label: "Biased Rejection",
        sublabel: "Hallucinated reason",
        kind: "output",
        icon: AlertTriangle,
        position: { x: X.col4, y: 60 },
        accent: "rose",
      },
      ...
      {
        id: "rai_guardrail",
        label: "PII & Bias Guardrail",
        sublabel: "Strips name, gender, ZIP",
        kind: "agent",
        icon: Shield,
        position: { x: X.col3, y: 380 },
        accent: "sky",
      },
      {
        id: "rai_blind",
        label: "Blind Financial Eval",
        sublabel: "Decides on DTI + credit only",
        kind: "agent",
        icon: Calculator,
        position: { x: X.col4, y: 380 },
        accent: "teal",
      },
      {
        id: "rai_audit",
        label: "Explainability & Audit",
        sublabel: "Generates Reason for Decision",
        kind: "agent",
        icon: Search,
        position: { x: X.col4 + 220, y: 380 },
        accent: "indigo",
      },
      {
        id: "rai_fair",
        label: "Fair Approval",
        sublabel: "Auditable, regulator-ready",
        kind: "output",
        icon: CheckCircle2,
        position: { x: X.col5 + 200, y: 380 },
        accent: "emerald",
      },
    ],
    edges: [
      ...
      // Unsafe branch (red, dashed)
      {
        id: "rai_eA1",
        source: "rai_intake",
        target: "rai_unrestricted",
        label: "raw data (incl. ZIP)",
        variant: "danger",
        dashed: true,
      },
      {
        id: "rai_eA2",
        source: "rai_unrestricted",
        target: "rai_biased",
        label: "silent bias",
        variant: "danger",
        dashed: true,
      ...
      // Safe branch (green, solid)
      {
        id: "rai_eB1",
        source: "rai_intake",
        target: "rai_guardrail",
        label: "raw data",
        variant: "success",
      },
      {
        id: "rai_eB2",
        source: "rai_guardrail",
        target: "rai_blind",
        label: "redacted features",
        variant: "success",
      },
      {
        id: "rai_eB3",
      ...
      },
      {
        ...
        source: "rai_audit",
        target: "rai_fair",
        label: "explainable approval",
        variant: "success",
      },
    ],
    tour: [
      {
        nodeIds: ["rai_input", "rai_intake"],
        edgeIds: ["rai_e1"],
        ...
        realWorld:
          "Most production fintech onboarding APIs collect everything by default — the discipline is what you do with it next.",
      },
      {
        nodeIds: ["rai_unrestricted", "rai_biased"],
        edgeIds: ["rai_eA1", "rai_eA2"],
        ...
        realWorld:
          "Apple Card (2019) and several US lenders have been investigated for exactly this pattern — models trained on historical lending data that encoded redlining.",
      },
      {
        nodeIds: ["rai_guardrail"],
        edgeIds: ["rai_eB1", "rai_eB2"],
        ...
        realWorld:
          "Anthropic's guardrails, OpenAI's moderation endpoint, and AWS Bedrock Guardrails are all this exact pattern: a small, focused model that runs before the big one.",
      },
      {
        nodeIds: ["rai_blind", "rai_audit"],
        edgeIds: ["rai_eB3"],
        ...
        realWorld:
          "US Reg B requires a specific reason for adverse action within 30 days. EU AI Act Art. 14 requires human-interpretable output for high-risk systems. This pair satisfies both.",
      },
      {
        nodeIds: ["rai_biased", "rai_fair"],
        edgeIds: ["rai_eA2", "rai_eB4"],
        ...
        realWorld:
          "This branching test — running the same input down both paths and diffing the outputs — is how regulated AI teams actually validate guardrails in production (e.g. Stripe's Radar, Plaid's risk models).",
      },
    ],
  },
];
