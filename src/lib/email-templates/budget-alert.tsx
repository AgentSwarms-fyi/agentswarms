      ...
      <Body style={main}>
        <Container style={container}>
          ...
          <Text style={text}>
            {isExceeded ? (
              <>
                Your {siteName} AI spend for this month has reached your hard cap of{" "}
                <strong>{fmt(monthlyCapUsd)}</strong>. You've used <strong>{fmt(spentUsd)}</strong>{" "}
                so far. Agents may stop accepting new runs until you raise the cap or the next
                billing month begins.
              </>
            ) : (
              <>
                Heads up — you've spent <strong>{fmt(spentUsd)}</strong> of your{" "}
                <strong>{fmt(monthlyCapUsd)}</strong> monthly AI budget on {siteName} (
          ...
          <Text style={text}>
            You can raise the cap, change alert thresholds, or set per-agent daily limits from the
            Budgets &amp; Guardrails page.
          </Text>

          <Hr style={hr} />

          <Text style={footer}>
            You're getting this because spend alerts are enabled on your account. Turn them off any
            time in {siteName} → Budgets.
          </Text>
