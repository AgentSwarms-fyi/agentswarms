      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-sm text-muted-foreground">
            ...
            <code className="rounded bg-muted px-1 py-0.5 text-xs">skill.md</code> files) you can attach to any agent
            or swarm node. At run time, every selected skill is composed into the agent's effective system prompt — so
            the same agent can be specialised for different jobs just by swapping skills.
          </p>
        </div>
      ...
      <Tabs value={tab} onValueChange={setTab}>
                  ...
                  <CardHeader className="pb-2">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <div className="flex gap-1">
                        <Button size="icon" variant="ghost" onClick={() => handleEdit(s)} title="Edit">
                          <Pencil className="h-4 w-4" />
                        </Button>
                        <Button size="icon" variant="ghost" onClick={() => handleDelete(s.id)} title="Delete">
        ...
        <TabsContent value="samples" className="pt-4">
          <p className="mb-3 text-xs text-muted-foreground">
            Built-in skills are always available to every agent. They cannot be edited or deleted, but you can
            duplicate one into your library and customise the copy.
          </p>
          <div className="grid gap-3 lg:grid-cols-2">
            {SAMPLE_SKILLS.map((s) => (
              <Card key={s.id}>
                <CardHeader className="pb-2">
                  <div className="flex items-start justify-between gap-2">
                    <CardTitle className="text-base">{s.name}</CardTitle>
                    <Badge variant="secondary" className="h-5">sample</Badge>
                  </div>
                  <p className="text-xs text-muted-foreground">{s.description}</p>
                </CardHeader>
                <CardContent className="space-y-2">
