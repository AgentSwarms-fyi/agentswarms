...
import {
  BookMarked, Search, Plus, Pencil, Trash2, Lock, User as UserIcon, Copy, Eye,
} from "lucide-react";
...
import {
  BUILT_IN_PROMPTS, PROMPT_CATEGORIES, type PromptCategory,
...
function PromptsPage() {
  ...
  const [previewPrompt, setPreviewPrompt] = useState<{ title: string; description?: string | null; content: string } | null>(null);

  useEffect(() => { loadUserPrompts(); }, []);
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Production-grade system prompts you can drop into any agent. Built-ins ship with the app and can't be deleted.
          </p>
        </div>
        <Button onClick={openNew}><Plus className="h-4 w-4 mr-2" /> New Prompt</Button>
      </div>
              ...
              <Card key={p.id} className="border-border/50 flex flex-col">
                ...
                <CardContent className="flex-1 flex flex-col">
                  ...
                  <div className="flex gap-1">
                    <Button variant="outline" size="sm" className="flex-1 gap-1.5" onClick={() => setPreviewPrompt({ title: p.title, description: p.description, content: p.content })}>
                      <Eye className="h-3 w-3" /> Preview
                    </Button>
                    <Button variant="outline" size="sm" className="flex-1 gap-1.5" onClick={() => copyContent(p.content)}>
          ...
          {filteredUser.length === 0 ? (
            <Card className="border-dashed border-2 border-border/50">
              <CardContent className="flex flex-col items-center justify-center py-12">
                ...
                <Button onClick={openNew}><Plus className="h-4 w-4 mr-2" /> Create your first prompt</Button>
              </CardContent>
            </Card>
          ) : (
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {filteredUser.map((p) => (
                <Card key={p.id} className="border-border/50 flex flex-col">
                  <CardHeader className="pb-3">
                    <div className="flex items-start justify-between gap-2">
                      <CardTitle className="text-base leading-tight">{p.title}</CardTitle>
                      <Badge variant="default" className="shrink-0 text-[10px]">Mine</Badge>
                    </div>
                    {p.description && (
                      <p className="text-xs text-muted-foreground line-clamp-2">{p.description}</p>
                    )}
                  ...
                  <CardContent className="flex-1 flex flex-col">
                    ...
                    <div className="flex gap-1">
                      <Button variant="outline" size="sm" className="gap-1.5" onClick={() => setPreviewPrompt({ title: p.title, description: p.description, content: p.content })}>
                        <Eye className="h-3 w-3" /> Preview
                      </Button>
                      <Button variant="outline" size="sm" className="flex-1 gap-1.5" onClick={() => copyContent(p.content)}>
                        <Copy className="h-3 w-3" /> Copy
                      ...
                      <Button variant="ghost" size="sm" className="text-destructive" onClick={() => deletePrompt(p.id)}>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <p className="text-[10px] text-muted-foreground">
                Tip: a great system prompt names the role, defines the process step-by-step, gives output format, and lists what the agent should refuse.
              </p>
            </div>
          </div>
          <DialogFooter>
      ...
      <Dialog open={!!previewPrompt} onOpenChange={(o) => !o && setPreviewPrompt(null)}>
        <DialogContent className="max-w-3xl max-h-[85vh] flex flex-col">
          ...
          <DialogFooter>
            ...
            <Button onClick={() => { if (previewPrompt) { copyContent(previewPrompt.content); setPreviewPrompt(null); } }}>
