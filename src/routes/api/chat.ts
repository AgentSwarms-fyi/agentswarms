...
// Lightweight keyword-based retrieval over knowledge_documents.content.
// No embeddings table exists, so we score by tokenized term frequency.
async function retrieveCitations(opts: {
  authToken: string;
  agentId: string;
  query: string;
  topK?: number;
}): Promise<Citation[]> {
  const sb = getServerSupabase(opts.authToken);
  if (!sb) return [];

  // 1. Load agent (RLS scopes to user automatically with the auth token).
  ...
  const extraIds = Array.isArray(tools.knowledgeBaseIds)
    ? (tools.knowledgeBaseIds as unknown[]).filter((x): x is string => typeof x === "string")
    : [];
  const kbIds = Array.from(
    new Set([...(agent.knowledge_base_id ? [agent.knowledge_base_id] : []), ...extraIds]),
  );
  if (kbIds.length === 0) return [];

  // 2. Pull KB names + documents (RLS-scoped).
  ...
  // 3. Tokenize query into useful terms.
  const STOP = new Set([
    "the",
    "a",
    "an",
  ...
  // 4. Score each document and pick best snippet around the highest-density area.
  const SNIPPET_RADIUS = 280;
  type Scored = { doc: (typeof docs)[number]; score: number; snippet: string };
  const scored: Scored[] = [];
  ...
  for (const doc of docs) {
    ...
    // Score = sum of occurrences of each term (capped to keep doc-length bias low).
    let score = 0;
    let bestPos = -1;
    let bestLocalHits = 0;
    for (const term of terms) {
      ...
      while (from < lower.length) {
        ...
        if (idx === -1) break;
        count += 1;
        // Track local density for snippet selection.
        const windowStart = Math.max(0, idx - SNIPPET_RADIUS);
        const windowEnd = Math.min(lower.length, idx + SNIPPET_RADIUS);
        let localHits = 0;
        for (const t of terms) {
          if (lower.indexOf(t, windowStart) !== -1 && lower.indexOf(t, windowStart) < windowEnd) {
            localHits += 1;
          }
        }
      ...
      score += Math.min(count, 5); // cap per-term contribution
    }
  ...
  const top = scored.slice(0, Math.max(1, Math.min(opts.topK ?? 5, 8)));
...
async function recordTrace(opts: {
  ...
  const tokensIn = opts.upstreamTokensIn ?? trace.promptTokensApprox;
  const tokensOut = isImg ? 0 : (opts.upstreamTokensOut ?? approxTokens(assistantText));
    ...
    async start(controller) {
      try {
        ...
        try {
          ...
          const tIn = upstreamTokensIn ?? trace.promptTokensApprox;
          const tOut = isImg ? 0 : (upstreamTokensOut ?? approxTokens(assistantText));
          const cUsd = isImg
